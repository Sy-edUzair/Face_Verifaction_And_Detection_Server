        ...
        try:
            ...
            for filename, _ in saved_uploads[1:]:
                ...
                result = self.face_verifier.verify_against_reference(
                    reference_face, candidate_faces
                )
                if not result.is_match:
                    all_match = False

                # Save all candidate face crops
                face_infos: list[FaceInfo] = []
                for face in candidate_faces:
                    saved_path = self.storage.save_u_and_m_face(
                        face, is_match=result.is_match
                    )
                    face_infos.append(
                        FaceInfo(
...
async def test_pipeline():
    ...
    try:
        ...
        for filename, _ in [(p.name, p) for p in test_video_paths[1:]]:
            ...
            print(
                f"  - {filename}: {status} (Similarity: {result.similarity_score:.4f})"
            )

            # Save candidate faces
            for face in candidate_faces:
                saved_path = orchestrator.storage.save_u_and_m_face(
                    face, is_match=result.is_match
                )
                print(f"    • Face saved: {saved_path}")
